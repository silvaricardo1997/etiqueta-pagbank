...
# Run locally:  pip install streamlit PyPDF2
...
APP_DESC = (
    ...
    "(canto inferior esquerdo da etiqueta medido a partir da borda esquerda e o topo a partir da borda superior), "
...
def build_crop_and_fit(
    ...
    # Altura da página em mm (não assume A4 fixo)
...
with st.sidebar:
    ...
    st.caption("Use ponto decimal. Os valores abaixo são os que funcionaram no seu caso.")
    ...
    if not uploaded:
        ...
        try:
            ...
            crop_bytes, fit_bytes, debug = build_crop_and_fit(
                ...
                x_left_mm=x_left_mm, y_top_mm=y_top_mm,
                width_mm=width_mm, height_mm=height_mm,
                extra_top_mm=extra_top_mm, extra_right_mm=extra_right_mm,
                extra_left_mm=extra_left_mm, extra_bottom_mm=extra_bottom_mm,
            ...
            st.caption(debug)
            st.download_button("⬇️ Baixar recorte exato (_crop.pdf)", data=crop_bytes, file_name=f"{base}_crop.pdf", mime="application/pdf")
            st.download_button("⬇️ Baixar versão 100×150 mm (_fit.pdf)", data=fit_bytes, file_name=f"{base}_fit.pdf", mime="application/pdf")
        except Exception as e:
            st.error(f"Falha ao processar o PDF: {e}")
            st.stop()

st.divider()
st.markdown("""**Dicas para imprimir na térmica (Coibeu WKZ-80D):**\n- Selecione mídia **100×150 mm (10×15 cm)**.\n- Use **Tamanho real / 100%** (desative “Ajustar à página”).\n- Desative margens automáticas do driver.\n""")
